...
export const BUMPS = ['none', 'minor', 'major'];
/**
...
const RANK = { none: 0, minor: 1, major: 2 };
const worst = (bumps) => bumps.reduce((acc, bump) => (RANK[bump] > RANK[acc] ? bump : acc), 'none');
/** Enum values as a set: two spellings of the same set are the same set. */
const enumValues = (spec) => spec.enum_values ?? [];
/**
 ...
 */
export function diffTypeSpec(from, to) {
    const before = canonicalizeTypeSpec(from).spec;
    const after = canonicalizeTypeSpec(to).spec;
    if (before.name !== after.name) {
        throw new TypeError(`cannot diff '${before.name}' against '${after.name}': different type names are different types, not versions of one type`);
    }
    const changes = [];
