...
describe('the check is not vacuous', () => {
  it('reads every source file in the package', () => {
    ...
    // landed, and when `output-verdict.ts` (asc-6ola.6) landed, which is the behaviour wanted -- a file added without being added
  ...
  it('actually finds fs imports to judge', () => {
    ...
    // would certify a package it never read. Measured 2026-09-15: the reader is
    // the only file that touches the filesystem at all, and the other six are
    // pure. `derive.ts` in particular reads no file: it takes records as
    // arguments, which is what lets its rules be tested against object literals.
