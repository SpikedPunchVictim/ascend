 ...
 */
import { canonicalizeTypeSpec, definitionShape, diffTypeSpec, typeHash, } from '@ascend/core';
...
const toStorage = (spec, options) => {
    ...
    for (const property of spec.properties) {
        if (property.description !== undefined)
            prose[property.name] = property.description;
    }
 ...
 */
export function registerType(db, spec, options) {
    ...
    const latest = db
        .prepare('SELECT version, major, spec_json FROM entry_types WHERE name = ? ORDER BY version DESC LIMIT 1')
        .get(shape.name);
    let version = 1;
    let major = 1;
    let bump = 'major';
    ...
    if (latest !== undefined) {
        ...
        const previous = JSON.parse(latest.spec_json);
        ...
        if (diff.bump === 'none') {
            throw new Error(`type '${shape.name}' hashes differently from version ${String(latest.version)} but the ` +
    ...
    if (ownsTransaction)
        db.exec('BEGIN');
    try {
        db.prepare(`INSERT INTO entry_types
         (name, version, major, type_hash, spec_json, description, record_when, prose_json, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(shape.name, version, major, hash, JSON.stringify(shape), options.description ?? null, options.recordWhen ?? null, proseJson, options.registeredAt);
...
const rowToVersion = (row) => ({
    ...
    prose: row.prose_json === null ? {} : JSON.parse(row.prose_json),
 ...
 */
export function typeVersions(db, name) {
    const rows = db
 ...
 */
export function findType(db, name, version) {
    const row = version === undefined
        ? db.prepare(`${SELECT_VERSION} WHERE name = ? ORDER BY version DESC LIMIT 1`).get(name)
        : db.prepare(`${SELECT_VERSION} WHERE name = ? AND version = ?`).get(name, version);
 ...
 */
export function deprecateType(db, name) {
    const result = db
        .prepare("UPDATE entry_types SET status = 'deprecated' WHERE name = ? AND status <> 'deprecated'")
        .run(name);
 ...
 */
export function updateTypeProse(db, name, version, prose) {
    ...
    db.prepare(`UPDATE entry_types
        SET description = ?, record_when = ?, prose_json = ?
      WHERE name = ? AND version = ?`).run(prose.description === undefined ? existing.description : prose.description, prose.recordWhen === undefined ? existing.recordWhen : prose.recordWhen, Object.keys(nextProse).length === 0 ? null : JSON.stringify(nextProse), name, version);
}
