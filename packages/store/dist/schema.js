...
export const SCHEMA_VERSION = MIGRATIONS.reduce((highest, migration) => Math.max(highest, migration.version), 0);
...
export class NewerSchemaError extends Error {
    storeVersion;
    buildVersion;
    constructor(storeVersion, buildVersion) {
        super(`this store is at schema version ${String(storeVersion)} but this build of ascend only knows ` +
...
export function migrate(db, migrations = MIGRATIONS) {
    ...
    if (from > target)
        throw new NewerSchemaError(from, target);
    ...
    for (const migration of pending) {
        ...
        try {
            ...
            throw new Error(`migration ${String(migration.version)} (${migration.name}) failed and was rolled back: ${detail}`);
        }
